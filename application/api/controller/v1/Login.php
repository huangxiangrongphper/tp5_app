...
class Login extends Commen
{
    public function save()
    {
        ...
        {
            return show(config('code.error'),'验证码不合法或者密码不合法','',404);
        }

        if(!empty($param['code']))
        {

           $code = TengXunSms::getInstance()->checkSmsIdentify($param['phone']);

           if($code != $param['code'])
          {
           // $param['code'] = (new Aes())->decrypt($param['code']);
            return show(config('code.error'),'验证码不合法或者失效','',404);
          }
        ...
        if($user && $user->status == 1) {
            ...
            if(!empty($param['code'])) {
                // 第一次登录 注册数据
                $data['username'] = 'Flourishing粉-' . $param['phone'];
                $data['status'] = 1;
        ...
        {
            $result = [
                'token' => (new Aes())->encrypt($data['token']."||".$id),
            ];
